...
public interface PiscinaRepository extends JpaRepository<Piscina, Long> {
    ...
    void insertarPiscina(@Param("id") Long id, @Param("capacidad") String capacidad, @Param("profundidad") String profundidad, @Param("horario") java.sql.Date horario);

    @Modifying
    @Transactional
    @Query(value = "UPDATE piscinas SET CAPACIDAD = :capacidad, PROFUNDIDAD = :profundidad, HORARIO = :horario WHERE ID = :id", nativeQuery = true)
    void actualizarPiscina(@Param("id") Long id, @Param("capacidad") String capacidad, @Param("profundidad") String profundidad, @Param("horario") java.sql.Date horario);
